...
@wp.kernel
def _warp_compute_corrections_batched(
    predicted_positions: wp.array(dtype=wp.vec3),
    inv_masses: wp.array(dtype=wp.float32),
    quat_inv_masses: wp.array(dtype=wp.float32),
    inv_inertia: wp.array(dtype=wp.float32),
    ...
    """Compute corrections for all rods in a single launch.

    This batched version processes all edges across all rods.
    Phase 1 of batched parallel corrections.

    Args:
        predicted_positions: Concatenated predicted positions (read-only).
        inv_masses: Concatenated inverse masses.
        quat_inv_masses: Concatenated inverse rotational masses.
        inv_inertia: Concatenated inverse inertia tensors.
        jacobian_pos: Concatenated position Jacobians.
        jacobian_rot: Concatenated rotation Jacobians.
        delta_lambda: Concatenated Lagrange multiplier increments.
        lambda_sum: Concatenated accumulated multipliers (updated in-place).
    ...
    # Position correction for particle 0
    if inv_m0 > 0.0:
        dp0_x = _warp_jacobian_dot(jacobian_pos, edge, 0, dl0, dl1, dl2, dl3, dl4, dl5)
        dp0_y = _warp_jacobian_dot(jacobian_pos, edge, 1, dl0, dl1, dl2, dl3, dl4, dl5)
        dp0_z = _warp_jacobian_dot(jacobian_pos, edge, 2, dl0, dl1, dl2, dl3, dl4, dl5)
    ...
    # Position correction for particle 1
    if inv_m1 > 0.0:
        dp1_x = _warp_jacobian_dot(jacobian_pos, edge, 3, dl0, dl1, dl2, dl3, dl4, dl5)
        dp1_y = _warp_jacobian_dot(jacobian_pos, edge, 4, dl0, dl1, dl2, dl3, dl4, dl5)
        dp1_z = _warp_jacobian_dot(jacobian_pos, edge, 5, dl0, dl1, dl2, dl3, dl4, dl5)
    ...
    # Rotation correction for particle 0
    ...
    # Rotation correction for particle 1
